...
CXX=clang++
CXXFLAGS=-g --std=c++11 -stdlib=libc++ -Werror -Weverything -pedantic -Wno-c++98-compat -Wno-padded -Wno-float-equal `sdl-config --cflags`
#CXXFLAGS=-g --std=c++11 -stdlib=libc++ -Wno-c++98-compat -Wno-padded `sdl-config --cflags`
...
run: default
	gdb $(module_target) -batch -x scripts/gdbExec | scripts/formatBt
